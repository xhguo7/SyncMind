...
class SyncMindConfig:
    def __init__(
        ...
        self.resync_output_save_dir = os.path.dirname(os.path.abspath(__file__)).split('evaluation')[0] + f"evaluation/syncmind_outputs/{self.save_dir_name}/"
    ...
    def init_instance_path(
        ...
        if resync_method == 'independent':
            self.save_dir_name = f"{self.instance_set_name}_{resync_method}_budget{RESOURCE_AWARENESS['total_credit']}_code{RESOURCE_AWARENESS['coding_cost']}_max{max_iterations}turn_{self.agent_model}"
        else:
            self.save_dir_name = f"{self.instance_set_name}_{resync_method}_budget{RESOURCE_AWARENESS['total_credit']}_code{RESOURCE_AWARENESS['coding_cost']}_ask{RESOURCE_AWARENESS['asking_cost']}_max{max_iterations}turn_{self.agent_model}"
        self.resync_output_save_dir = os.path.dirname(os.path.abspath(__file__)).split('evaluation')[0] + f"evaluation/syncmind_{self.agent_model}_max{max_iterations}turn_budget{RESOURCE_AWARENESS['total_credit']}_code{RESOURCE_AWARENESS['coding_cost']}_ask{RESOURCE_AWARENESS['asking_cost']}/{self.save_dir_name}/"
