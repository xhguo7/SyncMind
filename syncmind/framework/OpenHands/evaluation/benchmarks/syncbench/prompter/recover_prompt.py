...
class Prompter(object):
    ...
    def init_prompt(self, out_of_sync_task, metadata, cost_awareness):
        ...
        self.total_budget = cost_awareness['total_budget']
        ...
        independent_instruction_list = [
            {
                ...
                "content": f"You are a helpful assistant." + 
                           ...
                           f"""\n**Important Notice** In the beginning, you have an initial budget of ${self.total_budget}. Before correctly propose a solution, each of your `(b) Proposing a solution` attempts costs ${self.coding_cost}. Meanwhile, although you may make as many `(a) Python environment interaction` attempts as you want at no cost, you have in total {self.max_iterations} attempts to complete this task. You will fail this task if you use up all your ${self.total_budget} budget or reach the maximum {self.max_iterations}-attempt limit without generating a correct `(b) Proposing a solution` response. Therefore, please arrange each of your actions wisely."""
            },
            {
                "role": "user", 
                "content": f"[Budget: ${self.total_budget}] Your revised `test_repo` failed execution test as follows:\n[initial execution error]\n{self.initial_error_log}"
            }
        ]
        ...
        collaborative_instruction_list = [
            {
                ...
                "content": f"You are a helpful assistant." + 
                           ...
                           f"""\n**Important Rules** {self.agent_cls_suffix()}""" + 
                           f"""\n**Important Notice** In the beginning, you have an initial budget of ${self.total_budget}. Before correctly propose a solution, each of your `(b) Proposing a solution` attempts costs ${self.coding_cost}, while each of your `(c) Asking for human's assistance` attempts costs ${self.asking_cost}. Meanwhile, although you may make as many `(a) Python environment interaction` attempts as you want at no cost, you have in total {self.max_iterations} attempts to complete this task. You will fail this task if you use up all your ${self.total_budget} balance or reach the maximum {self.max_iterations}-attempt limit without generating a correct `(b) Proposing a solution` response. Therefore, please arrange each of your actions wisely. """ + 
