...
from evaluation.syncmind.runs.runtime_eval import extract_agent_revised_code
...
from evaluation.syncmind.prompter.know_agent import KnowPrompter
from evaluation.syncmind.prompter.interact import openai_api_qa_interaction
    ...
    if instance.instance_id not in test_result_dict.keys():
        test_result_dict[instance.instance_id] = {
            ...
            'instance_info': {
                ...
                'total_credit': metadata.details['total_credit'], 
...
def get_fake_user_response(
    ...
    test_result['total_credit'] = metadata.details['total_credit']
    ...
    if (metadata.details['resync_method'] == 'collaborative') and ("[QUESTION]" in message_content):
        ...
        fake_user_response = f"[Balance: ${current_balance} Left] {know_answer}"
...
def save_current_turn_ipythonaction(
    ...
    last_action = state.history.get_last_action()
    ipython_code = last_action.code
    agent_thought = last_action.thought

    '''
    Env interaction
    '''
    current_balance = state.extra_data['current_balance']  # balance unchanged
    interaction_update = f"{last_action.source.upper()} [IPythonRunCellAction]:\n[IPythonCode]\n{ipython_code}\n[AgentThought]\n{agent_thought}\n\n"
...
def save_current_turn_cmdaction(
    ...
    '''
    current_balance = state.extra_data['current_balance']  # balance unchanged
    interaction_update = f"{last_action.source.upper()} [CmdRunAction]:\n[Command]\n{command}\n[AgentThought]\n{agent_thought}\n\n"
...
def save_current_turn_finishaction(
    ...
    last_action = state.history.get_last_action()
    agent_thought = last_action.message

    '''
    Agent Finish Action
    '''
    current_balance = state.extra_data['current_balance']  # balance unchanged
    interaction_update = f"{last_action.source.upper()} [{type(last_action)}]:\n[AgentThought]\n{agent_thought}\n\n" + f"---------- INTERACTION END ----------\n[Final Balance: ${current_balance}] Task Completed for Instance with ID `{instance.instance_id}`\n\n"
...
def save_current_turn_othermessageaction(
    ...
    '''
    current_balance = state.extra_data['current_balance']  # balance unchanged
    # user_response = f"""You are disallowed to conduct this type of actions: action type = {type(last_action)}\nYou CANNOT finish this task until the user confirm that your revised `test_repo` have passed unit test. Unit test MUST be conducted by user after you choose "Option (c)" and provide your answer (with your textual "error explanation" and "code revision plan") to user."""
    # user_response = f"""[Balance: {current_balance} Left] **WARNING: INVALID ACTION "{type(last_action)}"** Please provide your answer of error cause explanation and code revision plan. You CANNOT finish this task until the user confirm that your revised `test_repo` have passed unit test. Unit test MUST be conducted by user after you choose "Option (c)" and provide your answer (with your textual "error explanation" and "code revision plan") to user."""
