...
class TestCalleeDataConstructor:
    @pytest.fixture
    def mock_args(self):
        args = Mock()
    ...
    @patch('syncbench.constructor.callee.EnvConfig')
    @patch('syncbench.constructor.callee.CalleeConstructor')
    def test_all_data_construction(self, MockCalleeConstructor, MockEnvConfig, constructor, mock_args):
        """Test if all_data_construction processes all repos correctly"""
        mock_env_config = MockEnvConfig.return_value
        mock_benchmark = MockCalleeConstructor.return_value
        ...
        assert MockCalleeConstructor.call_count == 2
        assert mock_benchmark.extract_fm_from_in_test_object.call_count == 2
        assert len(constructor.all_fm_list) == 4

    @patch('syncbench.constructor.callee.EnvConfig')
    @patch('syncbench.constructor.callee.CalleeConstructor')
    def test_single_repo_data_construction(self, MockCalleeConstructor, MockEnvConfig, constructor):
        ...
        with patch.object(constructor, 'save_to_json') as mock_save:
            ...
            MockCalleeConstructor.assert_called_once()
            mock_benchmark.extract_fm_from_in_test_object.assert_called_once()
            
            # Verify save_to_json was called with correct parameters
            expected_save_path = f"{constructor.dataset_save_path}callee_{constructor.repo_id}_{constructor.repo_name}.json"
